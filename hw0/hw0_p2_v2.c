...
bool sol(int total_color_num, int R, int C, Board board, Clue Row_clue[MAXR], Clue Col_clue[MAXC],int nowstep)
{
    ...
    int nowR,nowC;
    // printf("nowstep:%d\n",nowstep);
    for (;nowstep < R * C;  nowstep++){
        nowR = nowstep / R;
        nowC = nowstep % R;

        board.matrix[nowR][nowC] = 1;
        board.now_Color_num++;
        if (!whether_valid(board, nowR, nowC, Row_clue, Col_clue, R, C)){
            board.now_Color_num--;
    ...
    assert(nowstep == R * C);
    if (board.now_Color_num == total_color_num && valid(board, Row_clue, Col_clue, R, C, total_color_num)){
